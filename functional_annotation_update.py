...
def get_protein_seq(strain_name):
    if strain_name == '180197':
        fasta_file = '180197_pilon4_nomt_masked.fas'
    else:
        fasta_file = strain_name + '_ordered_soft.fas'
    cmd = 'agat_sp_extract_sequences.pl -g /home/yangmeixin/asiaticum/annotations_pre/filtering_annotation_update_new/9_rename_annotation/{0}_final_renamed.gff3 -f /home/yangmeixin/asiaticum/soft_masked_genomes_220405/{1} -p -o /home/yangmeixin/asiaticum/annotations_pre/filtering_annotation_update_new/10_renamed_protein_file/{0}_protein.faa'.format(strain_name, fasta_file)
...
def fun_annotate(strain_name):        
        ...
        cmd = 'emapper.py --cpu 60 -i ../../10_renamed_protein_file/{0}_protein.faa -m diamond --dmnd_iterate yes --pfam_realign realign -o {0} --sensmode sensitive'.format(strain_name)
        check = subprocess.check_call(cmd, shell=True)
        if check == 0:
            print(strain_name + ' run successfully')
        else:
